...
def favoritesTable():
    ...
    if exists(Settings.DB_USERS_ROOT):
        Log.info(f'Favorites database: "{Settings.DB_USERS_ROOT}" found')
    else:
        Log.error(f'Favorites database: "{Settings.DB_USERS_ROOT}" not found')

        open(Settings.DB_USERS_ROOT, "x")

        Log.success(f'Favorites database: "{Settings.DB_USERS_ROOT}" created')
    Log.database(f"Connecting to '{Settings.DB_USERS_ROOT}' database")
    ...
    try:
        ...
        Log.info(f'Table: "favorites" found in "{Settings.DB_USERS_ROOT}"')

        connection.close()
    except Exception:
        Log.error(f'Table: "favorites" not found in "{Settings.DB_USERS_ROOT}"')
        ...
        Log.success(f'Table: "favorites" created in "{Settings.DB_USERS_ROOT}"')
